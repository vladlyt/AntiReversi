...
func (b *Board) GetCellEvenIfNotExist(row, col int) Cell {
	if row < 0 || row > 7 || col < 0 || col > 7 || b[row][col].IsBlackHole {
		return Cell{}
	}
	return b[row][col]
}
...
func (b *Board) GetOppositeCell(row, col, dRow, dCol int, color Color) (int, int) {
	for i, j := row+dRow, col+dCol; !IsOutOfRange(i) && !IsOutOfRange(j) && b[i][j].IsFilled && !b[i][j].IsBlackHole; i, j = i+dRow, j+dCol {
...
func (b *Board) CanDoTurn(turn Turn) bool {
	return !b[turn.Row][turn.Col].IsFilled && !b[turn.Row][turn.Col].IsBlackHole && b.HasEnemyCellNearThatCanTake(turn)
}

func (b *Board) HasTurns(color Color) bool {
	for i := 0; i < 8; i++ {
		for j := 0; j < 8; j++ {
			if !b[i][j].IsFilled && !b[i][j].IsBlackHole && b.HasEnemyCellNearThatCanTake(Turn{
				Row:   i,
				Col:   j,
				Color: color,
			}) {
...
func (b *Board) IsWon(color Color) bool {
	return b.GetCellColorCount(color) > b.GetCellColorCount(!color)
}


func (b *Board) HasFreeCell() bool {
	for i := 0; i < 8; i++ {
		for j := 0; j < 8; j++ {
			if !b[i][j].IsFilled && !b[i][j].IsBlackHole {
				return true
			}
		}
	}
...
func (b *Board) CheckTurn(turn Turn) error {
	if turn.SkipTurn {
		if b.HasTurns(turn.Color) {
			return fmt.Errorf("have turn to do, but send skipTurn")
		}
		return nil
	}
	...
	if cell.IsBlackHole {
		return fmt.Errorf("cell is blackhole")
	}
	if cell.IsFilled {
...
func (b *Board) GetCellColorCount(color Color) int {
	...
	for i := range b {
		for j := range b[i] {
			if !b[i][j].IsBlackHole && b[i][j].IsFilled && b[i][j].Color == color {
				count++
			}
		}
	}
...
func (b *Board) GetAllTurns(color Color) []Turn {
	turns := make([]Turn, 0)
	for i := 0; i < 8; i++ {
		for j := 0; j < 8; j++ {
			turn := Turn{
...
func (b *Board) Copy() Board {
	...
	for i := 0; i < 8; i++ {
		...
		for j := 0; j < 8; j++ {
			duplicate[i][j] = Cell{
				IsFilled:    b[i][j].IsFilled,
				IsBlackHole: b[i][j].IsBlackHole,
				Color:       b[i][j].Color,
			}
		}
	}
